      ...
      <main className="min-w-0 overflow-x-clip">
        ...
        <section className="mx-auto max-w-5xl px-4 pb-10 sm:px-5 sm:pb-14">
          <div className="mx-auto flex max-w-2xl flex-col gap-4">
            <div className="landing-step-card px-5 py-6 text-center sm:px-8 sm:py-8">
              <p className="section-label text-indigo-200/80">Request coverage</p>
              <h2 className="mt-2 text-xl font-bold text-white sm:text-2xl">Want an area added?</h2>
              <p className="mx-auto mt-3 max-w-lg text-sm leading-relaxed text-indigo-100/75 sm:text-base">
                Country road, small highway outside your city, or another city or town — send the
                area name and coordinates. We aim to add it within 24 hours.
              </p>
              <a
...
function LandingPageWithAuth() {
  ...
  // Only leave the homepage for a *recent* notification tap (iOS often opens `/`
  // instead of the push URL). Do NOT auto-send everyone to the dashboard — that
  // blocked Install App / browsing the landing page while signed in.
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled || !deepLink) return;
      if (
        deepLink.startsWith("/dashboard") ||
        deepLink.startsWith("/desk") ||
        deepLink.startsWith("/feed")
      ) {
  ...
  }, [accountReady, isSignedIn, sessionReplaced]);
